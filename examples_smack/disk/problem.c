...
void problem_init(int argc, char* argv[]){
    ...
    for (int i=0; i<numbins; i++) {
        ...
        spmass += (M_PI/6)*rho*pow(sizebins[i],3)*sdist[i];
    }
    ...
    system("rm -f coordinates.txt");
    system("rm -f sizedist.txt");
